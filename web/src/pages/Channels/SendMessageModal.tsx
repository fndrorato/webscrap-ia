...
const SendMessageModal: React.FC<SendMessageModalProps> = ({
  ...
  useEffect(() => {
    if (!isOpen) {
      // Clear the object URL when the modal closes to prevent memory leaks
      if (selectedFile) {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      await axios.post(`/api/v1/channels/${channelId}/send_message/`, formData, {
        headers: {
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          ...
          <textarea
            ...
            className="w-full p-2 border rounded-md h-48 resize-y dark:bg-gray-700 dark:text-white"
            rows={4}
          ></textarea>
        </div>
        {selectedFile && (
          <div className="mb-4">
            <img src={URL.createObjectURL(selectedFile)} alt="Preview" className="max-w-full h-auto rounded-md" />
          ...
          <div className="flex items-center justify-between">
            <div className="relative w-full">
              ...
              <textarea
                ...
                value={messageText}
                onChange={(e: React.ChangeEvent<HTMLTextAreaElement>) => setMessageText(e.target.value)}
                className="h-20 w-full border-none bg-transparent pl-12 pr-5 text-sm text-gray-800 outline-hidden placeholder:text-gray-400 focus:border-0 focus:ring-0 dark:text-white/90"
                rows={1}
            ...
            <div className="flex items-center">             
              <button 
                type="button" 
                onClick={() => document.getElementById('hiddenAttachmentInput')?.click()}
              ...
              <Button type="submit" disabled={loading} className="ml-3 flex h-9 w-9 items-center justify-center rounded-lg bg-brand-500 text-white hover:bg-brand-600 xl:ml-5">
                {loading ? (
